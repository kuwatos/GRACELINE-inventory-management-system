...
export async function createUserNotificationService(data: { notifId: number, targetId: number, additionalDescription: string }, tx?: any) {
  ...
  await Promise.all(
    affectedUsers.map(async (user: AffectedUser) => {
      await createUserNotification({ notifId: data.notifId, userId: user.id, targetId: data.targetId, additionalDescription: data.additionalDescription }, tx);
    }),
  );
}
